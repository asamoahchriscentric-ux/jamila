...
// Web video component using HTML5 video

const WebVideo = ({ source, style, isMuted, isLooping, onEnded, posterUrl }) => {
...
const fallbackCategoryCards = [

  {
    ...
    image: require('./assets/logo-only-1024x1024.png'),
...
const DEFAULT_CATEGORY_IMAGE = require('./assets/logo-only-1024x1024.png');
...
function CategoryCard({ category, cardWidth, currency, onAddToCart, onRemoveFromCart, cartItems = [], onViewDetails, isPhone, isUserDarkMode }) {
  ...
  const itemPrice = category.price || 0;
  ...
  const isSelected = useMemo(() => {
    ...
    return cartItems.some((item) => item.id === category.id);

  }, [cartItems, category.id]);
  ...
  const handleToggleCart = () => {
    ...
    if (isSelected) {
      ...
      onRemoveFromCart(category.id, 'unit');

    } else {

      // Add to cart

      onAddToCart(category, 'unit', itemPrice);
...
const fallbackFooterSections = [
...

  {
    ...
    footer_items: [
      ...
      { id: 'f4', label: 'Home', action_type: 'navigate', action_value: 'shop', sort_order: 10, icon_library: 'FontAwesome', icon_name: 'home' },

      { id: 'f5', label: 'About Us', action_type: 'alert', action_value: 'About JAMILA coming soon', sort_order: 20, icon_library: 'FontAwesome', icon_name: 'info-circle' },

      { id: 'f6', label: 'JAMILA Shop', action_type: 'navigate', action_value: 'shop', sort_order: 30, icon_library: 'FontAwesome', icon_name: 'shopping-bag' },

      { id: 'f7', label: 'Contact Us', action_type: 'alert', action_value: 'Contact Us coming soon', sort_order: 40, icon_library: 'FontAwesome', icon_name: 'envelope' }
...

  {
    ...
    footer_items: [
      ...
      { id: 'f8', label: 'Cart', action_type: 'navigate', action_value: 'cart', sort_order: 10, icon_library: 'FontAwesome', icon_name: 'shopping-cart' },

      { id: 'f9', label: 'Checkout', action_type: 'checkout', sort_order: 20, icon_library: 'FontAwesome', icon_name: 'credit-card' },

      { id: 'f10', label: 'Wishlist', action_type: 'alert', action_value: 'Wishlist coming soon', sort_order: 30, icon_library: 'FontAwesome', icon_name: 'heart' },

      { id: 'f11', label: 'Terms And Conditions', action_type: 'alert', action_value: 'Terms & Conditions coming soon', sort_order: 40, icon_library: 'FontAwesome', icon_name: 'file-text' }
...
// const isAdminPage = currentPage === 'adminLogin'; // removed duplicate
  ...
  // Location/Store Selection states
  ...
  const [adminLocationId, setAdminLocationId] = useState(null); // Store-specific admin location

  const [selectedAdminStoreFilter, setSelectedAdminStoreFilter] = useState(null); // For super admins to filter by store
  ...
  const loadSubcategories = async (categoryName) => {
    ...
    try {
      ...
      const categoryId = catNameToId[categoryName.toLowerCase()];
      ...
      if (error) {
        ...
        console.warn('⚠️ Failed to load subcategories:', error.message);
      ...
      if (data && data.length > 0) {
        ...
        const chips = data.map(sub => sub.name);

        setSubcategoryChips(chips);

        setSubcategoryData(data);

        console.log('📂 Loaded subcategories for', categoryName, ':', chips);
      ...
      console.warn('⚠️ Error loading subcategories:', err.message);
  ...
  const checkStoreAvailability = async (cartItems) => {
    ...
    try {
      ...
      // Fetch inventory for all cart items across all locations

      const { data: inventoryData, error } = await supabase

        .from('inventory')

        .select('location_id, product_id, stock_quantity')
      ...
      inventoryData.forEach(inv => {
        ...
        if (!inventoryByLocation[inv.location_id]) {

          inventoryByLocation[inv.location_id] = {};

        }

        inventoryByLocation[inv.location_id][inv.product_id] = inv.stock_quantity;

      });

      

      // Check which locations have all items in stock
      ...
      const availableLocations = locations.filter(location => {
        ...
        // Check if all cart items are available at this location

        return cartItems.every(item => {

          const availableStock = locationInventory[item.id] || 0;
      ...
      console.log('📍 Available stores for cart:', availableLocations.length);

      return availableLocations;

    } catch (err) {

      console.error('❌ Failed to check store availability:', err.message);

      return locations; // Return all locations if check fails
...
const fetchFooterData = async () => {
  ...
  const loadSupabaseData = async (showLoading = true) => {
    ...
    try {
      ...
      try {
        ...
        const subcatRes = await supabase.from('subcategories').select('*');

        if (subcatRes.data && subcatRes.data.length > 0) {

          console.log('📂 Subcategories loaded:', subcatRes.data.length);

        }

      } catch (err) {

        console.warn('⚠️ Could not load subcategories:', err.message);
  ...
  }, [currentPage, user, selectedAdminStoreFilter]);



  useEffect(() => {

    if (!settings.autoplay || heroSlides.length === 0) return;



    const currentSlide = heroSlides[currentHeroSlide];

    if (currentSlide?.video_url) return; // videos advance via onEnded



    const durationMs = (currentSlide?.duration_seconds || settings.default_duration_seconds) * 1000;
  ...
  }, [currentHeroSlide, heroSlides, settings]);
  ...
  const goToNextSlide = () => {
    ...
    setCurrentHeroSlide((prev) => {
      ...
      if (next >= heroSlides.length) {
        ...
        return settings.loop_carousel ? 0 : prev; // stop at last slide if looping is off
  ...
  // Preload hero slide videos and images on web
  ...
  const fetchCustomerOrders = async () => {
    ...
    try {
      ...
      // Fetch orders with order_items, product details, and location info
        ...
        .insert([

          {
            ...
            metadata: {
              ...
              store_name: selectedLocation?.store_name || 'Unknown',
  ...
  const checkAdmin = async (user) => {
    ...
    try {
      ...
      // Store admin's location_id (null = super admin)

      if (profile?.location_id) {

        setAdminLocationId(profile.location_id);

        setSelectedAdminStoreFilter(profile.location_id);

      } else {

        setAdminLocationId(null);

        setSelectedAdminStoreFilter(null);
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      if (isLoginMode) {
        ...
        console.log('Email length:', authEmail.trim().length);

        console.log('Password length:', authPassword.trim().length);
        ...
        alert('Welcome to JAMILA! Your account has been created.');
  ...
  const fetchAdminOrders = async () => {
    ...
    try {
      ...
      // Filter by location if store-specific admin or if super admin has selected a filter

      const locationFilter = selectedAdminStoreFilter || adminLocationId;
  ...
  const filteredCategories = useMemo(() => {
    ...
    return productCards.filter((card) => {
      ...
      // TODO: Add subcategory filtering when subcategory data is linked to products

      const matchesSubcategory = !activeSubcategory || true; // Will be implemented when products have subcategory_id

      return selected && matchesQuery && matchesSubcategory;

    });

  }, [activeCategory, activeSubcategory, productCards, search]);
  ...
  const adminAddProduct = async () => {
    ...
    const productRow = {
      ...
      default_size: newProduct.has_weights ? (newProduct.default_size || 'M') : null,
    ...
    try {
      ...
      setNewProduct({ name: '', price: '', has_weights: true, default_size: 'M', tag: '', category_name: '', description: '', image_url: '', stock_quantity: '' });
  ...
  const adminEditProduct = async () => {
    ...
    const updatedFields = {
      ...
      default_size: editingProduct.hasWeights ? (editingProduct.default_size || 'M') : null,
  ...
  const columnCount = width >= 1800 ? 9
    : width >= 1600 ? 8
    : width >= 1400 ? 7
    : width >= 1200 ? 6
    : width >= 900 ? 5
      ...
      <View style={[styles.header, isPhoneScreen && { paddingHorizontal: 12 }, { 
        ...
        <View style={[styles.headerActions, isPhoneScreen && { gap: 6 }]}>
          ...
          <Pressable style={styles.badgeWrap} onPress={openCart}>
            ...
            <FontAwesome name="shopping-bag" size={18} color={isUserDarkMode ? darkPalette.oxblood : palette.oxblood} />
      ...
      {isAccountPage ? (
        ...
        <ScrollView contentContainerStyle={{ flex: 1, backgroundColor: palette.background, justifyContent: 'center', alignItems: 'center', padding: 24, minHeight: 500 }} showsVerticalScrollIndicator={false}>
          ...
          <View style={{ width: '100%', maxWidth: 420, backgroundColor: '#fff', borderTopWidth: 4, borderTopColor: palette.oxblood, padding: 36, shadowColor: '#000', shadowOffset: {width: 0, height: 8}, shadowOpacity: 0.08, shadowRadius: 24 }}>
            ...
            <Text style={{ fontSize: 11, fontWeight: '700', letterSpacing: 3, color: palette.oxblood, marginBottom: 8 }}>JAMILA</Text>
        ...
        <View style={[styles.adminDashboardLayout, { backgroundColor: adm.bg }]}>
          ...
          <View style={[styles.adminSidebar, isCompactAdmin && { display: 'none' }]}>
            ...
            <Text style={styles.adminSidebarBrand}>JAMILA</Text>
              ...
              <View style={[styles.adminNewTable, { backgroundColor: adm.surface, borderColor: adm.border }]}>
                ...
                {productCards
                  ...
                  .filter(product => {
                    ...
                    // If store admin, only show products for their location

                    if (adminLocationId) {

                      // For now, show all products since products don't have location_id yet

                      // TODO: Add location_id to products table and filter here
                  ...
                  .slice(0, activeAdminTab === 'Dashboard' ? 5 : undefined)
                      ...
                      <View style={[{flex: 0.5, alignItems: 'flex-end', justifyContent: 'center'}, isCompactAdmin && { position: 'absolute', right: 20, top: 28 }]}>
                        ...
                        <Pressable onPress={() => {
                          ...
                          setEditingProduct({
                            ...
                            default_size: product.default_size || 'M',
                              ...
                              <View key={oi} style={{flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 10, borderTopWidth: oi > 0 ? 1 : 0, borderTopColor: '#F0F0F0', gap: 10}}>
                                ...
                                <View style={{flex: 1}}>
                                  ...
                                  {ord.locations && (
                                    ...
                                    <Text style={{fontSize: 11, color: palette.oxblood, marginTop: 2}}>

                                      📍 {ord.locations.store_name}
          ...
          >
            ...
            <View style={{
              ...
              <Text style={{ color: '#fff', fontSize: 16, fontWeight: '700', letterSpacing: 3 }}>JAMILA</Text>
                ...
                <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={true} contentContainerStyle={{ paddingBottom: 16 }}>
                  ...
                  <View style={{ padding: 16, gap: 10 }}>
                    ...
                    {[
                      ...
                      { label: '✅ Order Ready', msg: `Hi ${customerMsgModal?.name || 'there'}, your order from JAMILA is ready and will be delivered soon! 🛍️` },

                      { label: '🚚 Out for Delivery', msg: `Hi ${customerMsgModal?.name || 'there'}, great news! Your JAMILA order is on its way. Our rider will be with you shortly. 🏍️` },

                      { label: '✅ Delivered', msg: `Hi ${customerMsgModal?.name || 'there'}, your JAMILA order has been delivered. Thank you for shopping with us! 🙏` },

                      { label: '💬 Follow Up', msg: `Hi ${customerMsgModal?.name || 'there'}, this is JAMILA. How was your experience with us? We'd love to hear your feedback! 😊` },
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          {heroSlides[currentHeroSlide]?.video_url ? (

            Platform.OS === 'web' ? (

              <WebVideo

                source={{ uri: heroSlides[currentHeroSlide].video_url }}

                posterUrl={heroSlides[currentHeroSlide]?.image_url}
            ...
            <Image
              ...
              source={{ uri: heroSlides[currentHeroSlide]?.image_url }}
          ...
          <Animated.View style={[styles.heroTextWrap, { opacity: reveal, transform: [{ translateY: lift }] }]}>
            ...
            <Text style={styles.heroTitle}>
              ...
              {heroSlides[currentHeroSlide]?.title?.split('\n').map((line, i) => (
          ...
          <View style={{position: 'absolute', bottom: 24, width: '100%', flexDirection: 'row', justifyContent: 'center', gap: 8}}>
            ...
            {heroSlides.map((slide, i) => (
              ...
              <View key={slide.id} style={{width: i === currentHeroSlide ? 24 : 8, height: 8, borderRadius: 4, backgroundColor: i === currentHeroSlide ? '#fff' : 'rgba(255,255,255,0.4)', transition: 'all 0.3s ease'}} />
        ...
        {/* CarouselComponent hidden - data used for hero section only */}
        {/*
        <CarouselComponent
        ...
        */}
        ...
        {/* Subcategory Choice Chips */}

        {subcategoryChips.length > 0 && (

          <View style={{ marginTop: 12, marginBottom: 8 }}>

            <Text style={[styles.sectionTitle, { color: isUserDarkMode ? darkPalette.charcoal : palette.charcoal, marginBottom: 8 }]}>
            ...
            {(() => {
              ...
              const subchips = subcategoryData.map((subcat) => {

                const active = subcat.name === activeSubcategory;
                ...
                return (
                  ...
                  <Pressable
                    ...
                    onPress={() => setActiveSubcategory(active ? null : subcat.name)}
              ...
              if (isMobileOrTablet) {
                ...
                return (
                  ...
                  <View style={[styles.chipsScrollContent, { flexWrap: 'wrap', paddingBottom: 8, width: '100%', justifyContent: 'flex-start' }]}>
...
<View style={[styles.searchWrap, { 
      ...
      <View style={styles.bottomNav}>
        ...
        >
          ...
          <FontAwesome name="home" size={24} color={currentPage === 'shop' ? '#fff' : '#888'} />
        ...
        >
          ...
          <FontAwesome name="shopping-cart" size={24} color={cartModalVisible ? '#fff' : '#888'} />
        ...
        >
          ...
          <FontAwesome name="user" size={24} color={userAccountSheetVisible ? '#fff' : '#888'} />
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 14 }}>
              ...
              <View style={{ gap: 4 }}>
                ...
                {locationsLoading ? (
                  ...
                  <View style={{ padding: 12, backgroundColor: isUserDarkMode ? darkPalette.surface : '#FAFAFA', borderRadius: 8 }}>

                    <ActivityIndicator size="small" color={isUserDarkMode ? darkPalette.oxblood : palette.oxblood} />
                  ...
                  <View style={{ padding: 12, backgroundColor: isUserDarkMode ? darkPalette.surface : '#FAFAFA', borderRadius: 8 }}>
                    ...
                    <Text style={{ color: isUserDarkMode ? darkPalette.secondary : palette.secondary, fontSize: 12 }}>
                        ...
                        ? availableLocations.map((location) => (
                          ...
                          >
                            ...
                            <Text style={{ 
                              ...
                              🚚 Delivery: GH₵{location.delivery_fee}
              ...
              <View style={[styles.bottomSheetSummaryCard, {
                ...
                <Pressable style={[styles.checkoutBtn, {
                  ...
                  if (!user) {
                    ...
                    // Check store availability before opening checkout

                    const available = await checkStoreAvailability(cartItems);
          ...
          <View style={{ 
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 14 }}>
              ...
              <View style={{ gap: 4 }}>
                ...
                <Text style={{ fontSize: 11, fontWeight: '700', color: palette.secondary, letterSpacing: 0.8 }}>BASE PRICE (M size) *</Text>
              ...
              {newProduct.has_weights && (
                ...
                <View style={{ gap: 4 }}>
                  ...
                  <Text style={{ fontSize: 11, fontWeight: '700', color: palette.secondary, letterSpacing: 0.8 }}>DEFAULT SIZE</Text>

                  <TextInput

                    value={newProduct.default_size}

                    onChangeText={(txt) => setNewProduct({ ...newProduct, default_size: txt })}
                ...
                {editingProduct.hasWeights && (
                  ...
                  <View style={{ gap: 4 }}>
                    ...
                    <TextInput
                      ...
                      value={editingProduct.default_size || ''}

                      onChangeText={(txt) => setEditingProduct({ ...editingProduct, default_size: txt })}
          ...
          <View style={{
            ...
            <View style={{ paddingHorizontal: 24, paddingTop: 16, paddingBottom: 12, backgroundColor: '#FAFAFA' }}>
              ...
              <Text style={{ fontSize: 13, color: palette.secondary, marginTop: 6, lineHeight: 18 }}>
                ...
                {isLoginMode ? 'Sign in to access your orders and account.' : 'Join JAMILA for a seamless shopping experience.'}
